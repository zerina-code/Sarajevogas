report 50178 "Phys. Inventory List v1" //Customized version of Microsoft original report id 722
{
    DefaultLayout = RDLC;
    RDLCLayout = './PhysInventoryList_v1.rdl';
    ...
    dataset
    {
        dataitem(PageLoop; "Integer")
        {
            ...
            column(GroupByAccount; GroupByAccount)
            {
            }
            column(GroupByEmployee; GroupByEmployee)
            {
            }
        ...
        layout
        {
            area(content)
            {
                group(Options)
                {
                    ...
                    field(GroupByAccount; GroupByAccount)
                    {
                        ApplicationArea = Basic, Suite;
                        Caption = 'Group by Account';
                        ToolTip = 'Specifies if you want to group the report by Account.';
                    }
                    field(GroupByEmployee; GroupByEmployee)
                    {
                        ApplicationArea = Basic, Suite;
                        Caption = 'Group by Employee';
                        ToolTip = 'Specifies if you want to group the report by Employee.';
                    }
                }
            }
        }
