report 50143 "Group Transfer"
{
    // BH1.00, Maloprodajna kalkulacija
    DefaultLayout = RDLC;
    RDLCLayout = './GroupTransfer.rdl';
    ...
    dataset
    {
        dataitem(DataItem1; "Transfer Receipt Line")
        {
            ...
            begin
                ...
                Quantity6 := CalcGroupedQuantity(StartDate, EndDate, "Item No.");
                loc.reset();
                ...
                IF CurrentVE.FINDFIRST THEN BEGIN
                    ...
                    if selectedText = 'Wholesale' then
                    ...
                    REPEAT
                        if selectedText <> 'Wholesale' then begin
                            CurrentField7 := 0;
                            ...
                            // CurrentVE."T.Retail Unit Price with VAT";
                            ...
                            CurrentSellAmountWithVat := CurrentVE."T.Retail Unit Price with VAT";
                            //CurrentVE."Retail Unit Price with VAT" * Quantity6;
                            sumField15 := CurrentSellAmountWithVat;
                            sumField17 := CurrentValueSellAmountWithoutVat;
                            sumField18 += CalcGroupedVATVP(StartDate, EndDate, "Item No.", CurrentVE."Location Code"); //ovdje djemina za Veleprodaju
            ...
            begin
                ...
                IF selectedText = 'Retail' then
                    DataItem1.SetFilter("Transfer-to Code", 'CNG MLP')
                ELSE
                    DataItem1.SetFilter("Transfer-to Code", 'CNG VLP|VLASTITA');
    ...
    begin
        CRL.Reset();
        CRL.SetFilter("Report ID", '%1', 50143);
    ...
    begin
        ...
        IF selectedText = 'Retail' then
            selectedTextM := 'Prenos iz veleprodaje'
        else
            selectedTextM := 'Interni dokument-Zapisnik (ulaznica za veleprodaju)'
    ...
    var
        ...
        Selected: Option "Retail","Wholesale";
        Year: Integer;
        Control: Text;
    ...
    begin
        ...
        if trLine.FindSet() then
            repeat
                cnt += 1;
            until trLine.Next = 0;
        if sel <> 'Wholesale' then exit(1) else exit(cnt);
    end;
    ...
    procedure CalcGroupedPriceWithVat(var receiptDate: Date; endDate: Date; itemNo: Text): Decimal;
    var
        trLine: Record "Transfer Receipt Line";
        amount: Decimal;
        amountWithVAT: Decimal;
    ...
    begin
        ...
        if trLine.FindSet() then
            repeat
                ...
                IF CurrentVE.FINDFIRST THEN BEGIN
                    REPEAT
                        if CurrentVE."Cost per Unit" * CurrentVE."invoiced Quantity" <> 0 then
                            PriceDiff := ((CurrentVE."Total Retail Amount" - (CurrentVE."Cost per Unit" * CurrentVE."invoiced Quantity")) / (CurrentVE."Cost per Unit" * CurrentVE."invoiced Quantity")) * 100
    ...
    //CurrentVE."Cost per Unit" * Quantity6

    procedure CalcGroupedVATVP(var receiptDate: Date; endDate: Date; itemNo: Text; Loc: Code[20]): Decimal;
    var
        trLine: Record "Transfer Receipt Line";
        VATamount: Decimal;
        CurrentVE: Record "Value Entry";
        ...
        if trLine.FindSet() then
            repeat
                ...
                IF CurrentVE.FINDFIRST THEN BEGIN
                    ...
                    VATamount := (CurrentVE."Cost per Unit" * Quantity6) * (abs(CurrentVE."Retail VAT") / 100);
    ...
    begin
        ...
        ELSE
            controlTrLine.SetFilter("Transfer-to Code", 'CNG VLP|VLASTITA');
        ...
        if controlTrLine.FindFirst() then begin
            if controlTrLine."Group Calculation Number" = '' then begin
                ...
                ELSE
                    number := NoSeriesMgt.GetNextNo(Gener."Group Retail Calculation Entry Series VP", TODAY, true);
                ...
                if trLine.FindSet() then
                    repeat
                        if trLine."Group Calculation Number" = '' then begin
                            ...
                            trLine.Modify();
                            trHeader.GET(trLine."Document No.");
                            trHeader."Group Calculation Number" := number;
                            trHeader.Modify();
                        end;
                    until trLine.Next = 0;
