report 50225 "Calculate Depreciation Job"
{
                ...
                with GenJnlLine do begin
                    ...
                    if TempGenJnlLine.Find('-') then
                        repeat
                            ...
                            //dodati
                            Fixed.Reset();
                            Fixed.SetFilter("No.", '%1', TempGenJnlLine."Account No.");
                            if Fixed.FindFirst() then begin
                                FADeprBook.Reset();
                                FADeprBook.SetFilter("Depreciation Book Code", '%1', GenJnlLine."Depreciation Book Code");
                                FADeprBook.SetFilter("FA No.", '%1', TempGenJnlLine."Account No.");
                                if FADeprBook.FindFirst() then begin
                                    FaPosting.Get(FADeprBook."FA Posting Group");
                                    if Fixed."Donation Percentage" <> 0 then begin
                                        TempGenJnlLine.Validate(Donation, FaPosting."Depreciation Expense Acc. D.");
                                        TempGenJnlLine.Validate("Donation Percentage", Fixed."Donation Percentage");
                                        TempGenJnlLine.Validate(OS, TempGenJnlLine."Account No.");
                                    end
                                    else begin
                                        TempGenJnlLine.Donation := '';
                                        TempGenJnlLine."Donation Percentage" := 0;
                                        TempGenJnlLine.OS := '';

                                    end;
                                    GenJnlLine.Donation := TempGenJnlLine.Donation;
                                    GenJnlLine."Donation Percentage" := TempGenJnlLine."Donation Percentage";
                                    GenJnlLine.Validate(OS, TempGenJnlLine."Account No.");


                                    if (GenJnlLine."Donation Percentage" = 1) then begin
