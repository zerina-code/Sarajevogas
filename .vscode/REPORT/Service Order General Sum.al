report 50217 "Service Order General Sum"
{
    ...
    dataset
    {
        ...
        {
            RequestFilterFields = "Request type", "Responsible Department", "Finishing Date (filter)";
            ...
            column(Request_Type; ServiceHeaderAdd."Request Type") { }
            column(Document_DateGEO; format(ServiceHeaderAdd."Document Date", 0, '<day,2>.<month,2>.<year4>')) { }
            column(Responsible_Department_NameGEO; ServiceHeaderAdd."Responsible Department Name") { }
            column(Request_Department_NameGEO; ServiceHeaderAdd."Request Department Name") { }
            column(Geo__Activity_TypeGEO; ServiceHeaderAdd."Geo. Activity Type") { }
            column(Name; ServiceHeaderAdd.Name) { }
            column(Real__Process__Empl__Name; ServiceHeaderAdd."Real. Process. Empl. Name") { }
            column(Starting_DateGEO; ServiceHeaderAdd."Starting Date") { }
            column(Starting_TimeGEO; ServiceHeaderAdd."Starting Time") { }
            //column(Implementation_Time; "Implementation Time") { }
            column(Finishing_Date; ServiceHeaderAdd."Finishing Date") { }
            column(Finishing_TimeGEO; ServiceHeaderAdd."Finishing Time") { }
            column(AddressGEO; ServiceHeaderAdd.Address) { }
            column(Municipality_Name; ServiceHeaderAdd."Municipality Name") { }

            column(Real__Verif__Empl__NameGEO; ServiceHeaderAdd."Real. Verif. Empl. Name") { }
            ...
            column(Status_request; Status_request) { }
            ...
            column(Topon; ServiceHeaderAdd."Topcon OS 201") { }
            ...
            dataitem("Service Line ResourceEmp"; "Service Line RN")
            {
                ...
                column(BrojacLjudi2; BrojacLjudi2) { }
            ...
            var
                ...
                ServiceLineRN2: Record "Service Line RN";
            ...
            begin
                ...
                if SHCZKOrg.findfirst then begin
                    ...
                    if SHCZk.findfirst then begin
                        ...
                        while STRPOS(TempTextKomentar, 'Komentar:') > 0 do begin
                            StartPos := STRPOS(TempTextKomentar, 'Komentar:');
                            EndPos := STRPOS(CopyStr(TempTextKomentar, StartPos + 9), 'Napomena:');
                            if EndPos = 0 then
                                EndPos := STRLEN(TempTextKomentar) - StartPos + 1;
                            TekstKomentar := TekstKomentar + 'Komentar: ' + DELCHR(COPYSTR(TempTextKomentar, StartPos + 9, EndPos - 9), '<>', ' ') + ' ';
                        ...
                        while STRPOS(TempTextNapomena, 'Napomena:') > 0 do begin
                            StartPos := STRPOS(TempTextNapomena, 'Napomena:');
                            EndPos := STRPOS(CopyStr(TempTextNapomena, StartPos + 9), 'Komentar:');
                            if EndPos = 0 then
                                EndPos := STRLEN(TempTextNapomena) - StartPos + 1;
                            TekstNapomena := TekstNapomena + 'Napomena: ' + DELCHR(COPYSTR(TempTextNapomena, StartPos + 9, EndPos - 9), '<>', ' ') + ' ';
              ...
              then
                ...
                //EK
                if (ServiceHeaderAdd.Status_request = ServiceHeaderAdd.Status_request::"Completely Realized") or
    (ServiceHeaderAdd.Status_request = ServiceHeaderAdd.Status_request::"Realized with deadline") then
                    Status_request := 1
                else
                    Status_request := 0;
                ...
                if Operator = '' then begin
                    ServiceLineAll.reset;
                    ServiceLineAll.copyfilters("Service Item Line");
                    if ServiceLineAll.findset then
                        repeat
                            ServiceLineRN2.Reset();
                            ServiceLineRN2.SetFilter("Document No.", '%1', ServiceLineAll."Document No.");
                            ServiceLineRN2.SetFilter("Request Resource Type1", '%1', ServiceLineRN."Request Resource Type1"::"Team Leader");
                            ServiceLineRN2.SetFilter("Resource No.", '%1', VodjaGrupeReq);
                            if ServiceLineRN2.FindFirst() then begin
                                ServiceLineRN.reset;
                                ServiceLineRN.SetFilter("Document No.", '%1', ServiceLineAll."Document No.");
                                ServiceLineRN.SetFilter("Request Resource Type1", '<>%1', ServiceLineRN."Request Resource Type1"::"Team Leader");
                                if ServiceLineRN.FindSet() then
                                    repeat

                                        //ena
                                        if not OperatorDict.ContainsKey(ServiceLineRN."Resource Name") then begin
                                            OperatorDict.Add(ServiceLineRN."Resource Name", true);
                            ...
                            RNVodja.Reset();
                            RNVodja.SetFilter("Document No.", '%1', ServiceLineAll."Document No.");
                            RNVodja.SetFilter("Document Type", '%1', ServiceLineAll."Document Type");
                            RNVodja.SetFilter("Request Resource Type1", '%1', RNVodja."Request Resource Type1"::"Team Leader");
                            RNVodja.SetFilter("Resource No.", '%1', VodjaGrupeReq);
                            if RNVodja.findfirst then begin
                                ServiceLineRN.reset;
                                ServiceLineRN.SetFilter("Document No.", '%1', ServiceLineAll."Document No.");
                                ServiceLineRN.SetFilter("Request Resource Type1", '<>%1', ServiceLineRN."Request Resource Type1"::"Team Leader");
                                if ServiceLineRN.FindSet() then
            ...
            trigger OnPreDataItem()
            var
                ProcessedDocuments: Dictionary of [Code[20], Boolean];
            begin
                Comp.get;
                ...
                group(Option)
                {
                    field(ReportLayout; ReportLayout)
                    {
                        ...
                        begin
                            ...
                            CustomReportLayout.SetFilter("Report ID", '%1', 50217);
    ...
    begin
        CL.Reset();
        CL.SetFilter("Report ID", '%1', 50217);
 ...
 DO BEGIN
    ...
    var
        ...
        SHeader: Record "Service Header";
        RNVodja: record "Service Line RN";
        NedostaciRMS: Text;
        NedostaciRMSNE: Text;
...
;
        ...
        ServiceLineAll: record "Service Item line";
        ...
        OperatorDict: Dictionary of [Text, Boolean];
